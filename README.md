...
Area Chair appointments are maintained in `service/area-chairs.yml` and appear first. Reviewer records remain in the separate conference and journal files. The PDF groups service by these roles, then by year, using the same editable category titles as the website.
...
The full publication list groups years from newest to oldest. Within each year, papers appear in this order: First / Co-first, Corresponding / Last, then Other. Roles are derived from the author list using the names in `_config.yml` → `scholar`: the first listed author or an author marked `*` / `∗` counts as first/co-first, `†` denotes corresponding authorship, and the final listed author counts as last author. A paper with multiple roles sorts into the earliest matching group; citation keys provide a stable order within each group. This ordering is generated into the peer-reviewed and preprint bibliographies, so it also works without JavaScript. Selected Publications retain their configured `cv_order`.

The role filter offers All, First / Co-first, Corresponding / Last, and Other. A paper may match multiple role filters; Other means none of the first, corresponding, or last roles. The same filter applies to the separate preprints section, while Selected Publications always remain visible. Awards are displayed as text in both lists. Every role button and Bib toggle supports keyboard access; hiding empty year groups keeps the filtered list compact.
...
`Teaching & Mentoring` is one CMS entry with independent `Teaching` and `Mentoring` items. The Teaching page renders only `kind: instructor` sections; TA and other history remain in the source and CV. The Supervision page reads the shared mentoring list, including links, periods, and outcomes. Its current openings and requirements are maintained in `_data/content/supervision.yml`; the homepage keeps a shorter recruitment summary in `_data/content/home.yml`. `Work Experience` uses main jobs and subjobs: roles at the same institution belong under one main job, while a different institution requires a new main job. `CV Profile` belongs to the `CV` collection; shared identity data such as name, email, and avatar lives only in `Website Profile` to avoid duplication.

Homepage research directions store their introduction, diagram caption/icon, subcategories, and publication citation keys in `home.yml`. The diagrams and cards both follow this list; add or reorder directions without editing templates. Badges use shared venue metadata and link to peer-reviewed publications. The CMS exposes the same nested structure.

The shared contact email has one editable source: `profile.yml` → `basics.email`. The homepage, applications, HTML CV, PDF CV, and generated social data all use it. Shared page links are in `site.yml` → `links`. Page titles, navigation settings, section headings, and display labels live in `pages.yml`; `content_pages.rb` applies this metadata before page layouts, navigation, and SEO render. Page templates contain presentation logic, while these files own the editable copy. Creating an entirely new layout or drawing a new icon still requires a component change; normal content updates do not.

The homepage news limit is configurable in `pages.yml` → `home.news_limit`; news content stays in `_news/*.md`. Most homepage honors reference the CV award title, preserving one source for award names and dates. Website biography and recruitment copy can describe an upcoming appointment without changing the existing CV. The legacy `/talk/`, `/news/`, and `/cv/` routes remain available. Teaching remains unpublished until its page is explicitly enabled.

Run `python3 scripts/build_cv_content.py validate` before building. Validation checks research citation keys, unique direction IDs, supported icons, page-copy references, and recruitment fields. Run `python3 -m unittest discover -s scripts -p test_faculty_content.py` to check CMS field coverage, email propagation, and adding research directions. Generated files and the PDF should always be rebuilt from these sources.
