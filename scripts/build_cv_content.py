...
Editable sources:
  ...
  * `_data/content/home.yml`, `supervision.yml`, and `pages.yml`
...
def validate_profile(data: Any) -> dict[str, Any]:
    ...
    for field in ("name_local", "subtitle", "body", "image", "align"):
    ...
    socials = mapping(profile.get("socials", {}), "profile.socials")
    # The editable email lives in basics; legacy social renderers receive a
    # derived copy so the website, JSON resume, and PDF cannot drift apart.
    socials["email"] = basics.get("email", "")
...
def validate_site(data: Any) -> dict[str, Any]:
    ...
    links = mapping(settings.get("links"), "site.links")
    for key in ("cv", "openings", "publications", "news"):
        value = text(links.get(key), f"site.links.{key}", required=True)
        if not re.fullmatch(r"/(?!/)[^\s\\{}]*", value):
            validate_url(value, f"site.links.{key}", allow_empty=False)
...
def service_item_text(item: dict[str, Any], *, omit_role: str = "Reviewer") -> str:
    name = item.get("name") or item.get("venue") or item.get("title") or ""
    rendered = markdown_link_or_text(str(name), item.get("url"))
    role = text(item.get("role"), "service.role")
    return f"{rendered} ({latex_escape(role)})" if role and role.lower() != omit_role.lower() else rendered
...
def render_service(service: dict[str, Any]) -> str:
    ...
    section_roles = {
        "area-chairs": "Area Chair",
        "conference-reviewers": "Reviewer",
        "journal-reviewers": "Reviewer",
        "tutorial": "Tutorial Speaker",
    }
    for group in service.get("groups", []):
        entries = group.get("entries", [])
        if not entries:
            continue
        lines.append(rf"\yearSeparator{{{latex_escape(group['title'])}}}")
        by_year: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for item in entries:
            by_year[text(item["year"], "service.year")].append(item)
        for year in sorted(by_year, key=year_sort_key, reverse=True):
            items: list[str] = []
            for item in sorted(by_year[year], key=lambda entry: entry["name"].lower()):
                if group["id"] == "recognition":
                    value = rf"\textbf{{{markdown_link_or_text(item['name'], item.get('url'))}}}"
                    if item.get("role"):
                        value += f": {latex_escape(item['role'])}"
                else:
                    value = service_item_text(item, omit_role=section_roles.get(group["id"], ""))
                items.append(value.rstrip("."))
            lines.append(rf"\serviceLine{{{latex_escape(year)}}}{{{'; '.join(items)}.}}")
    lines.extend([r"\end{rSection}", ""])
    return "\n".join(lines)
...
def validate_faculty_content(home: Any, supervision: Any, pages: Any, publications: list[dict[str, Any]]) -> None:
    ...
    published_keys = {paper["key"] for paper in publications if not paper["is_preprint"]}
    seen_ids: set[str] = set()
    for index, value in enumerate(directions):
        path = f"home.directions[{index}]"
        direction = mapping(value, path)
        identifier = text(direction.get("id"), f"{path}.id", required=True)
        if not re.fullmatch(r"[a-z][a-z0-9-]*", identifier) or identifier in seen_ids:
            validation_error(f"{path}.id", "must be a unique lowercase identifier")
        seen_ids.add(identifier)
        for key in ("title", "description", "stage_label", "illustration_description"):
            text(direction.get(key), f"{path}.{key}", required=True)
        if direction.get("illustration") not in {"data", "models", "agents"}:
            validation_error(f"{path}.illustration", "must select an existing data, models, or agents icon")
        for subindex, subvalue in enumerate(list_value(direction.get("subcategories"), f"{path}.subcategories")):
            subpath = f"{path}.subcategories[{subindex}]"
            subcategory = mapping(subvalue, subpath)
            text(subcategory.get("title"), f"{subpath}.title", required=True)
            for key in validate_string_list(subcategory.get("publications"), f"{subpath}.publications"):
    ...
    for index, criterion in enumerate(list_value(supervision.get("criteria"), "supervision.criteria")):
        path = f"supervision.criteria[{index}]"
        criterion = mapping(criterion, path)
        for key in ("title", "description"):
            text(criterion.get(key), f"{path}.{key}", required=True)
    contact = mapping(supervision.get("contact"), "supervision.contact")
    for key in ("subject", "introduction"):
        text(contact.get(key), f"supervision.contact.{key}", required=True)
    validate_string_list(contact.get("questions"), "supervision.contact.questions")

    for key, value in pages.items():
        if key == "schema_version":
            continue
        copy = mapping(value, f"pages.{key}")
        text(copy.get("title"), f"pages.{key}.title", required=True)
        if not isinstance(copy.get("nav"), bool):
            validation_error(f"pages.{key}.nav", "must be true or false")
        for field in ("nav_order", "news_limit"):
            number = copy.get(field)
            if number is not None and (type(number) is not int or number < 1):
                validation_error(f"pages.{key}.{field}", "must be a positive integer")

    # Validate the fields actually consumed by templates, so deleting a label
    # produces an actionable error instead of silently rendering blank text.
    templates = list((REPOSITORY_ROOT / "_pages").glob("*.md"))
    templates += list((REPOSITORY_ROOT / "_layouts").glob("*.liquid"))
    templates += list((REPOSITORY_ROOT / "_includes").glob("*.liquid"))
    for template in templates:
        source = template.read_text(encoding="utf-8")
        match = re.search(r"^content_key: (\w+)$", source, re.MULTILINE)
        key = match.group(1) if match else ("home" if template.name == "about.liquid" else None)
        if key:
            copy = mapping(pages.get(key), f"pages.{key}")
            for field in re.findall(r"page\.copy\.(\w+)", source):
                if field not in copy:
                    validation_error(f"pages.{key}.{field}", f"required by {template.name}")
        for group, field in re.findall(r"site\.data\.content\.pages\.(\w+)\.(\w+)", source):
            if field not in mapping(pages.get(group), f"pages.{group}"):
                validation_error(f"pages.{group}.{field}", f"required by {template.name}")
...
def load_and_validate() -> tuple[Any, ...]:
    ...
    validate_faculty_content(
        load_yaml(CONTENT_DIRECTORY / "home.yml"),
        load_yaml(CONTENT_DIRECTORY / "supervision.yml"),
        load_yaml(CONTENT_DIRECTORY / "pages.yml"),
        content[-1],
    )
    return content
